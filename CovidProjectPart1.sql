...
Select DTH.continent, DTH.location, DTH.date, DTH.population, VAC.new_vaccinations, SUM(convert(bigint, VAC.new_vaccinations)) OVER (Partition by DTH.location Order by DTH.location, DTH.date ROWS UNBOUNDED PRECEDING) as RollingCountVaccinated
